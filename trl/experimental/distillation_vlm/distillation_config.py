...
class DistillationConfig(_BaseConfig):
    ...
    Parameters:
        ...
        > Parameters that control multi-component distillation losses (qvac additions)

        use_rdist (`bool`, *optional*, defaults to `False`):
            Enable Relation Distillation (RDist) from LLaVA-KD (ICCV 2025). Distils pairwise visual token similarity
            structure from teacher to student. Architecture-agnostic across hidden dim mismatches.
        alpha (`float`, *optional*, defaults to `0.5`):
            Weight for the SFT cross-entropy loss component when combining the SFT, KL and RDist objectives.
        beta_kl (`float`, *optional*, defaults to `0.3`):
            Weight for the KL divergence distillation loss component when combining the SFT, KL and RDist objectives.
        beta_rdist (`float`, *optional*, defaults to `0.2`):
            Weight for the RDist feature-level distillation loss component when combining the SFT, KL and RDist
            objectives.
        teacher_conf_threshold (`float`, *optional*, defaults to `0.7`):
            Confidence threshold below which a backup teacher is used (Stage 3+). Not used in Stage 2.
    """
    ...
    # ── qvac additions: multi-component distillation losses ─────────────────────────────────────────────
    # These fields are used by `DistillationTrainer` to combine an SFT cross-entropy loss with the existing
    # KL/JSD distillation loss and the optional Relation Distillation (RDist) feature-level loss from
    # LLaVA-KD (ICCV 2025). They are additive: when `use_rdist=False` and the `alpha`/`beta_kl` weights are
    # left at their defaults, the loss combination still degenerates to the original distillation loss
    # behaviour (the trainer only switches to the combined loss when these are explicitly used).
    use_rdist: bool = field(
        default=False,
        metadata={
            "help": "Enable Relation Distillation (RDist) from LLaVA-KD (ICCV 2025). Distils pairwise "
            "visual token similarity structure from teacher to student. Architecture-agnostic across "
            "hidden dim mismatches."
        },
    )
    alpha: float = field(
        default=0.5,
        metadata={"help": "Weight for the SFT cross-entropy loss component."},
    )
    beta_kl: float = field(
        default=0.3,
        metadata={"help": "Weight for the KL divergence distillation loss component."},
    )
    beta_rdist: float = field(
        default=0.2,
        metadata={"help": "Weight for the RDist feature-level distillation loss component."},
    )
    teacher_conf_threshold: float = field(
        default=0.7,
        metadata={
            "help": "Confidence threshold below which backup teacher is used (Stage 3+). "
            "Not used in Stage 2."
        },
    )
