...
class _DistillationCollator:
    ...
    def __call__(self, examples: list[dict[str, Any]]) -> dict[str, torch.Tensor]:
        ...
        # --- qvac addition: VLM image passthrough ---
        # If the dataset already contains pre-processed image tensors (e.g. produced upstream by a
        # `ProcessorMixin`), forward them through the batch as-is so VLM students can consume
        # them. Text-only datasets are unaffected because these keys simply won't be present.
        # Covers the keys emitted by current HF VLM processors:
        #   - `pixel_values`           — universal
        #   - `pixel_attention_mask`   — SmolVLM family
        #   - `image_grid_thw`         — Qwen2/2.5-VL
        #   - `image_sizes`            — LLaVA-Next
        #   - `mm_token_type_ids`      — Qwen2.5-VL
        # Stacking assumes per-sample tensors share a shape (one image per sample, etc.). For
        # multi-image / variable-shape batches, supply a custom `data_collator` that handles
        # padding/concat for these keys (see `examples/scripts/distill_vlm/train_stage2.py`).
        for key in (
            "pixel_values",
        ...
        # --- qvac addition: per-sample class weights for weighted KL ---
        # Stage 2 of the VLM distillation pipeline uses class-frequency weighting (the medical
...
class DistillationTrainer(_BaseTrainer):
    ...
    def __init__(
        ...
        # --- qvac addition: explicit tokenizer for VLM workflows ---
        # When `processing_class` is a [`~transformers.ProcessorMixin`] (e.g. SmolVLM2's processor),
        # the underlying text tokenizer (`processor.tokenizer`) can be passed here so the data
        # collator and pad-token logic still work. For text-only setups this is left as `None` and
        # we alias it to `processing_class` below for backwards compatibility.
        ...
        # --- qvac addition: processing_class alias ---
        # Allow VLM users to pass `processing_class=processor` and `tokenizer=processor.tokenizer`.
        # For text-only callers `tokenizer` is None and we alias it to `processing_class` so the
        # rest of the trainer can keep treating a single object as the tokenizer.
        if processing_class is not None and tokenizer is None:
        ...
        # --- qvac addition: RDist hook buffers ---
        # Always allocate the lists so `compute_loss` can probe them safely. Hooks are only
        # registered when `use_rdist=True` to avoid the runtime cost of an unused forward hook.
        self._s_feat = [None]
        self._t_feat = [None]
        if args.use_rdist:
            self._register_rdist_hooks()
    ...
    # --- qvac addition: RDist forward hooks ---
    def _register_rdist_hooks(self):
        """Register forward hooks that capture the last vision-encoder layer's output for RDist.

        The student path assumes a SmolVLM-family architecture
        (`model.model.vision_model.encoder.layers[-1]`). The teacher path assumes the same
        SigLIP-family layout when a local teacher is available; if a teacher's structure differs,
        inspect with ``list(dict(self.teacher_model.named_modules()).keys())[:60]`` and update this
        method. When the teacher is served via vLLM (no `teacher_model`), only the student hook is
        registered and `_t_feat[0]` stays `None`, which `compute_loss` treats as "skip RDist".
        """
        try:
        ...
        if self.teacher_model is not None:
            try:
                ...
                warnings.warn(
                    f"Could not register teacher RDist hook: {e}\n"
                    f"Run: list(dict(teacher.named_modules()).keys())[:60] "
                    f"and update _register_rdist_hooks manually."
                )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Student forward pass — forward image tensors when present so VLM students work.
        # See `_DistillationCollator.__call__` for the list of supported VLM keys; these are
        # forwarded verbatim to the student model so its own forward() signature can consume
        # whichever keys it needs (the rest are silently ignored by transformers).
        ...
        for key in (
            ...
            if key in inputs:
                student_forward_kwargs[key] = inputs[key]
        student_outputs = model(**student_forward_kwargs)
        ...
        # --- qvac addition: class-frequency weighted KL ---
        # When the dataset provides per-sample class weights (e.g. inverse class frequency for
        # imbalanced datasets — the medical example uses this), rescale the KL term by their
        # normalized mean. With a scalar
        # `kl_loss` this is a no-op in expectation; it becomes meaningful when subclasses move to a
        # per-sample reduction. Kept here so the hook is wired regardless of reduction strategy.
        kl_loss = loss
        if "class_weight" in inputs:
            w = inputs["class_weight"].to(kl_loss.device).float()
            kl_loss = (kl_loss * w).mean() / w.clamp(min=1e-6).mean()

        # --- qvac addition: RDist ---
        # Relation Distillation (LLaVA-KD, ICCV 2025) compares pairwise visual-token similarity
        ...
        if (
            ...
            t_sim = torch.bmm(t, t.transpose(1, 2))
            rdist_val = F.mse_loss(s_sim, t_sim)

        # --- qvac addition: combined loss + per-component logging ---
        # Backwards compat: when `use_rdist=False` we keep returning the original distillation loss
        # exactly. When `use_rdist=True` we explicitly combine SFT + KL + RDist using the configured
        # weights and log each component so users can monitor them separately.
        if self.args.use_rdist:
            shifted_logits = student_outputs.logits[:, prompt_length - 1 : -1, :]
            ...
            self._metrics[mode]["loss/kl"].append(float(kl_loss.detach()))
            self._metrics[mode]["loss/rdist"].append(float(rdist_val.detach()))

            # Reset hook buffers so the next forward pass cannot accidentally reuse stale features.
            self._s_feat[0] = None
            self._t_feat[0] = None
        else:
            loss = kl_loss
